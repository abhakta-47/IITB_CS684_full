...
void forward() { motion(HIGH, LOW, HIGH, LOW); }

void backward() { motion(LOW, HIGH, LOW, HIGH); }

void right() { motion(HIGH, LOW, LOW, HIGH); }

void left() { motion(LOW, HIGH, HIGH, LOW); }

void forward_right() { motion(HIGH, LOW, LOW, LOW); }
...
void stop() { motion(LOW, LOW, LOW, LOW); }

void brake() { motion(HIGH, HIGH, HIGH, HIGH); }
...
void port_expander_init() { Wire.begin(); }
...
void beep_on() { PCF8574Write(0xDF & PCF8574Read()); }

void beep_off() { PCF8574Write(0x20 | PCF8574Read()); }
